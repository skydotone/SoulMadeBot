const execute = async (interaction, options) => {
  ...
  if (onOrOff === 'join') {
    ...
    await interaction.reply({ content: "You have been given the " + `<@&${roleId}>` + " role.", ephemeral: true });
  } else {
    interaction.member.roles.remove(roleId).catch((e) => console.log(e));
    await interaction.reply({ content: `You removed the <@&${roleId}> role.`, ephemeral: true });
  }
}
